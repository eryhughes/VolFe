...
def initial_guesses(run,PT,melt_wf,setup,species,models,system): ### CHECK ###
    ...
    if models.loc["insolubles","option"] == "H2O-CO2 only":
        guessx = mg.xg_CO2(run,PT,melt_wf,setup,species,models)
    
    if system in ["COFe","HOFe","SOFe","CHOFe","SHOFe","SCOHFe","SCHOFe","CHOXFe"]:
        guessw = 0.
    ...
    if system in ["COFe","HOFe","SOFe"]:
        ...
        if starting_P == "set":
            ...
            guessy = mg.xg_CO(run,PT,melt_wf,setup,species,models) 
            guessz = mg.xg_X(run,PT,melt_wf,setup,species,models)
        ...
        if solve_species in ["OCS","scipy"]:
            if starting_P == "set":
                ...
                guessy = mg.xg_CO(run,PT,melt_wf,setup,species,models) 
                guessz = mg.xg_S2(run,PT,melt_wf,setup,species,models)
        elif solve_species == "OHS":
            if starting_P == "set":
                guessy = setup.loc[run,"xg_H2"]
                guessz = setup.loc[run,"xg_S2"]
            else:
                guessy = mg.xg_H2(run,PT,melt_wf,setup,species,models) 
                guessz = mg.xg_S2(run,PT,melt_wf,setup,species,models)
            ...
            if starting_P == "set":
                ...
                guessy = mg.xg_CO(run,PT,melt_wf,setup,species,models) 
                guessz = mg.xg_H2(run,PT,melt_wf,setup,species,models)
        if system == "SCHOXFe":
            if starting_P == "set":
                guessw = setup.loc[run,"xg_X"]
            else:
                guessw = mg.xg_X(run,PT,melt_wf,setup,species,models) 
    return guessx, guessy, guessz, guessw

def mg_equilibrium(run,PT,melt_wf,bulk_wf,setup,species,models,nr_step,nr_tol,guessx,guessy,guessz,guessw): ### CHECK ###
    system = set_system(melt_wf,models)
    solve_species = models.loc["solve_species","option"]
    
    if system in ["COFe","SOFe"]: # no H
        wt_H_, xg_H2_, xg_H2O_, xg_CH4_, xg_H2S_, xm_H2O_, wm_H2O_, wm_H2_, wm_CH4_, wt_g_H = 0.,0.,0.,0.,0.,0.,0.,0.,0.,""
    if system in ["COFe","HOFe","CHOFe","CHOXFe"]: # no S
    ...
    if system == "COFe": 
        if models.loc["solve_species","option"] == "scipy":
            sweet = eq_COFe_TEST(run,PT,bulk_wf,melt_wf,species,setup,models,guessx)
        if models.loc["insolubles","option"] == "no":
            xg_O2_,A,B,C = eq_COFe(run,PT,bulk_wf,melt_wf,species,setup,models,nr_step,nr_tol,guessx) # COFe system
        ...
        xg_O2_,A,B,C = eq_HOFe(run,PT,bulk_wf,melt_wf,species,setup,models,nr_step,nr_tol,guessx) # HOFe system
        ...
        xg_O2_,A,B,C = eq_HOFe_xenia(run,PT,bulk_wf,species,setup,models,nr_step,nr_tol,guessx) # HOFe system
        ...
        if models.loc["insolubles","option"] == "no":
            xg_O2_, xg_CO_, A,B,C = eq_CHOFe(run,PT,bulk_wf,melt_wf,species,setup,models,nr_step,nr_tol,guessx,guessy) # CHOFe system
            ...
            xg_CO2_, A,B,C = eq_CH(run,PT,bulk_wf,melt_wf,species,setup,models,nr_step,nr_tol,guessx) # CHOFe system
            xg_CO2_, xg_H2O_, xm_H2O_, xm_CO2_, Xm_t, Xg_t, wm_H2O_, wm_CO2_ = A
            mbCH, wt_g_C, wt_g_H = B
            wt_g, wt_O_, wt_C_, wt_H_ = C
            guessx = xg_CO2_
        elif models.loc["insolubles","option"] == "yes":
            xg_O2_, xg_CO_, A,B,C = eq_CHOFe_2(run,PT,bulk_wf,melt_wf,species,setup,models,nr_step,nr_tol,guessx,guessy) # CHOFe system
        ...
        if models.loc["insolubles","option"] == "no" and models.loc["H2S_m","option"] == "no":
            xg_O2_, xg_S2_, A,B,C, = eq_SHOFe(run,PT,bulk_wf,melt_wf,species,setup,models,nr_step,nr_tol,guessx,guessy) # SHOFe system
            ...
            A,B,C,D = eq_SHOFe_2(run,PT,bulk_wf,melt_wf,species,setup,models,nr_step,nr_tol,guessx,guessy)
        ...
        if models.loc["insolubles","option"] == "no" and models.loc["H2S_m","option"] == "no":
            ...
            A,B,C,D = eq_SCHOFe_2(run,PT,bulk_wf,melt_wf,species,setup,models,nr_step,nr_tol,guessx,guessy,guessz,solve_species) # SHOFe system
            if solve_species == "OCS":
                xg_O2_, xg_CO_, xg_S2_ = A
                guessx, guessy, guessz = xg_O2_, xg_CO_, xg_S2_
            elif solve_species == "OHS":
        ...
        if models.loc["insolubles","option"] == "no":
            ...
            A,B,C,D = eq_CHOXFe(run,PT,bulk_wf,melt_wf,species,setup,models,nr_step,nr_tol,guessx,guessy,guessz,solve_species) # SHOFe system
        ...
        if models.loc["insolubles","option"] == "no" and models.loc["H2S_m","option"] == "no":
            ...
            xg_O2_,xg_CO_,xg_S2_,xg_X_,xg_H2_,xg_H2O_,xg_CO2_,xg_OCS_,xg_SO2_,xg_H2S_,xg_CH4_,Xg_t,wm_CO2_,wm_CH4_,wm_CO_,wm_H2O_,wm_H2_,wm_SO3_,wm_S_,wm_H2S_,wm_ST_,wm_X_,xm_H2O_,xm_CO2_,Xm_t,Fe32,Fe3T,S62,S6T,wt_g = eq_SCHOXFe(run,PT,bulk_wf,melt_wf,species,setup,models,nr_step,nr_tol,guessx,guessy,guessz,guessw,solve_species) # SHOXFe system
            Xm_t_ox,mbX, mbY, mbZ, wt_g_O, wt_g_C, wt_g_H, wt_g_S, mbW, wt_g_X,wt_O_, wt_C_, wt_H_, wt_S_, wt_X_ = "","","","","","","","","","","","","","",""
    
    return xg_O2_, xg_H2_, xg_S2_, xg_H2O_, xg_CO_, xg_CO2_, xg_SO2_, xg_CH4_, xg_H2S_, xg_OCS_, xg_X_, Xg_t, xm_H2O_, xm_CO2_, Xm_t, wm_H2O_, wm_H2_, wm_CO2_, wm_CO_, wm_CH4_, wm_S_, wm_SO3_, wm_H2S_, wm_ST_, wm_X_, Fe32, Fe3T, S62, S6T, wt_g_O, wt_g_C, wt_g_H, wt_g_S, wt_g_X, wt_g, wt_O_, wt_C_, wt_H_, wt_S_, wt_X_, guessx, guessy, guessz, guessw
...
def eq_C_melt(run,PT,melt_wf,species,setup,models): # equilibrium partitioning of C in the melt in CO system
    ...
    K2 = mdv.C_CO3(run,PT,melt_wf,setup,species,models) # mole fraction
    K3 = (mdv.C_CO(run,PT,melt_wf,setup,species,models))/1000000. # weight fraction
    M_C = species.loc['C','M']
...
def eq_H_melt(run,PT,melt_wf,species,setup,models,nr_step,nr_tol): # equilibrium partitioning of H in the melt in HO system
    ...
    K2 = mdv.C_H2O(run,PT,melt_wf,setup,species,models) # mole fraction
    K3 = mdv.C_H2(run,PT,melt_wf,setup,species,models) # ppm
    M_H = species.loc['H','M']
...
def eq_CH_melt(run,PT,melt_wf,species,setup,models,nr_step,nr_tol,guessx,guessy):
    ...
    K4_ = mdv.C_H2O(run,PT,melt_wf,setup,species,models) # mole fraction
    K5_ = mdv.C_CO3(run,PT,melt_wf,setup,species,models) # mole fraction
    K6_ = mdv.C_H2(run,PT,melt_wf,setup,species,models) # ppm
    K7_ = mdv.C_CO(run,PT,melt_wf,setup,species,models) # ppm
    K8_ = mdv.C_CH4(run,PT,melt_wf,setup,species,models) # ppm
...
def eq_HS_melt(run,PT,melt_wf,species,setup,models,nr_step,nr_tol): # not sure this is right?
    ...
    K2_ = mdv.C_H2O(run,PT,melt_wf,setup,species,models) # mole fraction
    K3_ = mdv.C_S(run,PT,melt_wf,setup,species,models)/1000000. # weight fraction
    K4_ = mdv.C_SO4(run,PT,melt_wf,setup,species,models)/1000000. # weight fraction
    K5_ = mdv.C_H2S(run,PT,melt_wf,setup,species,models)/1000000. # weight fraction
    K6_ = mdv.KHOg(PT,models) 
    K7_ = mdv.C_H2(run,PT,melt_wf,setup,species,models)/1000000. # weight fraction
...
def eq_CHS_melt(run,PT,melt_wf,species,setup,models,nr_step,nr_tol,guessx,guessy,guessz):
    ...
    K11_ = mdv.C_CO3(run,PT,melt_wf,setup,species,models) # mole fraction
    K12_ = mdv.C_CO(run,PT,melt_wf,setup,species,models)/1000000. # weight fraction
    K13_ = mdv.C_CH4(run,PT,melt_wf,setup,species,models)/1000000. # weight fraction
...
def eq_SOFe_melt(run,PT,bulk_wf,melt_wf,species,setup,models,nr_step,nr_tol,guessx): # equilibrium between S and Fe in the melt
    P = PT["P"]
    wt_O = bulk_wf['O']
    wt_S = bulk_wf['S']
    wt_Fe = bulk_wf['Fe']
    ...
    # equilibrium constants
    K2_ = mdv.C_S(run,PT,melt_wf,setup,species,models)/1000000.
    K3_ = mdv.C_SO4(run,PT,melt_wf,setup,species,models)/1000000.
    ...
    M_m_ = mg.M_m_ox(run,melt_wf,setup,species,models)
...
def eq_COFe(run,PT,bulk_wf,melt_wf,species,setup,models,nr_step,nr_tol,guessx):
    P = PT["P"]
    wt_O = bulk_wf['O']
    wt_C = bulk_wf['C']
    wt_Fe = bulk_wf['Fe']
    ...
    K2_ = mdv.C_CO3(run,PT,melt_wf,setup,species,models)
    KD1_, KD2, y = mdv.FefO2_KC91_EqA_terms(run,PT,melt_wf,setup,species,models)
    ...
    constants = [P, wt_O, wt_C, wt_Fe, K1_, K2_, KD1_, KD2, y, y_CO2_, y_O2_, y_CO_, M_C, M_O, M_Fe, M_O2, M_CO, M_CO2, M_FeO, M_FeO15, M_m_]
    ...
    def mg_COFe(xg_O2_):
        ...
        return xg_CO2_, xg_CO_, xm_CO2_, Xm_t, Xg_t, Fe32, Fe3T, wm_CO2_
    
    def mb_COFe(xg_O2_):
        xg_CO2_, xg_CO_, xm_CO2_, Xm_t, Xg_t, Fe32, Fe3T, wm_CO2_ = mg_COFe(xg_O2_)
        diff,wt_g_O,wt_g_C = f_COFe(xg_O2_)
        wt_g = (wt_g_O+wt_g_C)/2
        wt_C_ = M_C*((wt_g*(((xg_CO2_+xg_CO_)/Xg_t) - (xm_CO2_/Xm_t)))+(xm_CO2_/Xm_t))
        wt_O_ = M_O*((wt_g*(((2.0*xg_CO2_ + xg_CO_ + 2.0*xg_O2_)/Xg_t)-(2.0*xm_CO2_/Xm_t)))+(2.0*xm_CO2_/Xm_t) + (wt_Fe/M_Fe)*((1.5*Fe32+1.0)/(Fe32+1.0)))
        return wt_g, wt_O_, wt_C_
    
    def f_COFe(xg_O2_):
        xg_CO2_, xg_CO_, xm_CO2_, Xm_t, Xg_t, Fe32, Fe3T, wm_CO2_ = mg_COFe(xg_O2_)
        wt_g_C = ((wt_C/M_C) - (xm_CO2_/Xm_t))/(((xg_CO2_+xg_CO_)/Xg_t) - (xm_CO2_/Xm_t))
        wt_g_O = ((wt_O/M_O) - (2.0*xm_CO2_/Xm_t) - (wt_Fe/M_Fe)*((1.5*Fe32+1.0)/(Fe32+1.0)))/(((2.0*xg_CO2_ + xg_CO_ + 2.0*xg_O2_)/Xg_t)-(2.0*xm_CO2_/Xm_t))
        diff = wt_g_C - wt_g_O
...
def eq_COFe_TEST(run,PT,bulk_wf,melt_wf,species,setup,models,guessx):
    P = float(PT["P"])
    ...
    # equilibrium constants
    K1_ = float(mdv.KCOg(PT,models))
    K2_ = float(mdv.C_CO3(run,PT,melt_wf,setup,species,models))
    KD1_, KD2, y = mdv.FefO2_KC91_EqA_terms(run,PT,melt_wf,setup,species,models)
    KD1_ = float(KD1_)
    KD2 = float(KD2)
    y = float(y)
    ...
    M_m_ = float(mg.M_m_SO(run,melt_wf,setup,species))
     
    # system
    def solve_system(xg_O2_):
        xg_CO2_ = (1.0-xg_O2_)/(1.0 + (y_CO2_/(K1_*y_CO_*(y_O2_*xg_O2_*P)**0.5)))
        xg_CO_ = (y_CO2_*xg_CO2_)/(K1_*y_CO_*(y_O2_*xg_O2_*P)**0.5)
        xm_CO2_ = K2_*y_CO2_*xg_CO2_*P
        Xm_t = xm_CO2_*M_CO2 + (1.0-xm_CO2_)*M_m_
        ...
        Fe3T = Fe32/(1+Fe32)
        wm_CO2_ = (xm_CO2_*M_CO2)/Xm_t
        C = ((wt_C/M_C) - (xm_CO2_/Xm_t))/(((xg_CO2_+xg_CO_)/Xg_t) - (xm_CO2_/Xm_t))
        O = ((wt_O/M_O) - (2.0*xm_CO2_/Xm_t) - (wt_Fe/M_Fe)*((1.5*Fe32+1.0)/(Fe32+1.0)))/(((2.0*xg_CO2_ + xg_CO_ + 2.0*xg_O2_)/Xg_t)-(2.0*xm_CO2_/Xm_t))
        return C - O
    
    print(guessx)
    guessx = float(guessx)
    print(guessx)
    guessx = optimize.fsolve(solve_system, guessx)
    print(guessx)
...
def eq_HOFe(run,PT,bulk_wf,melt_wf,species,setup,models,nr_step,nr_tol,guessx):
    P = PT["P"]
    wt_O = bulk_wf['O']
    wt_H = bulk_wf['H']
    wt_Fe = bulk_wf['Fe']
    ...
    # equilibrium constants
    K1_ = mdv.KHOg(PT,models)
    K2_ = mdv.C_H2O(run,PT,melt_wf,setup,species,models)
    K3_ = mdv.C_H2(run,PT,melt_wf,setup,species,models)/1000000.
    KD1_, KD2, y = mdv.FefO2_KC91_EqA_terms(run,PT,melt_wf,setup,species,models)
...
def eq_HOFe_xenia(run,PT,bulk_wf,melt_wf,species,setup,models,nr_step,nr_tol,guessx):
    P = PT["P"]
    wt_O = bulk_wf['O']
    wt_H = bulk_wf['H']
    wt_Fe = bulk_wf['Fe']
...
def eq_SHOFe(run,PT,bulk_wf,melt_wf,species,setup,models,nr_step,nr_tol,guessx,guessy):
    P = PT["P"]
    wt_O = bulk_wf['O']
    wt_S = bulk_wf['S']
    wt_H = bulk_wf['H']
    ...
    K4_ = mdv.C_H2O(run,PT,melt_wf,setup,species,models)
...
def eq_SHOFe_2(run,PT,bulk_wf,melt_wf,species,setup,models,nr_step,nr_tol,guessx,guessy):
    P = PT["P"]
    wt_O = bulk_wf['O']
    wt_S = bulk_wf['S']
    wt_H = bulk_wf['H']
    ...
    K13_ = mdv.C_H2(run,PT,melt_wf,setup,species,models)/1000000.0
    K14_ = mdv.C_H2S(run,PT,melt_wf,setup,species,models)/1000000.0
    KD1_, KD2, y = mdv.FefO2_KC91_EqA_terms(run,PT,melt_wf,setup,species,models)
    ...
    M_FeO15 = species.loc['FeO1.5','M']
    M_m_ = mg.M_m_SO(run,melt_wf,setup,species)
    M_m_ox = mg.M_m_ox(run,melt_wf,setup,species,models)
...
def eq_SCOFe(run,PT,bulk_wf,melt_wf,species,setup,models,nr_step,nr_tol,guessx,guessy):
    ...
    def mg_SCOFe(xg_O2_,xg_CO_):
        xg_CO2_ = (K2_*y_CO_*xg_CO_*(y_O2_*xg_O2_*P)**0.5)/y_CO2_
        a = (1/(y_S2_*P))*(y_SO2_/(K2_*y_O2_xg_O2_))**2.0
        b = 1.0 + (((y_CO_*x_CO)**3.0*y_SO2*P)/(K3_*y_OCS_*(xg_CO2_*y_CO2_)**2.0))
    ...
    def df_SCOFe(xg_O2_,xg_CO_,constants):
        dmbSO_O = de.SCOFe_SO_O2(xg_O2_,xg_CO_,constants)
        dmbSO_S = de.SCOFe_SO_CO(xg_O2_,xg_CO_,constants)
        dmbSH_O = de.SCOFe_SC_O2(xg_O2_,xg_CO_,constants) 
        dmbSH_S = de.SCOFe_SC_CO(xg_O2_,xg_CO_,constants)
        return dmbSO_O, dmbSO_S, dmbSH_O, dmbSH_S
    
    xg_O2_, xg_CO_ = jac_newton(guessx,guessy,f_SCOFe,df_SCOFe,nr_step,nr_tol)
    results1 = mg_SCOFe(xg_O2_,xg_CO_)
    results2 = f_SCOFe(xg_O2_,xg_CO_)
    results3 = mb_SCOFe(xg_O2_,xg_CO_)
    #print(wt_O, wt_S, wt_H, xg_O2, xg_S2, results1, results2, results3)
...
def eq_CHOFe(run,PT,bulk_wf,melt_wf,species,setup,models,nr_step,nr_tol,guessx,guessy):
    P = PT["P"]
    wt_O = bulk_wf['O']
    wt_C = bulk_wf['C']
    wt_H = bulk_wf['H']
    ...
    K3_ = mdv.KCOHg(PT,models)
    K4_ = mdv.C_H2O(run,PT,melt_wf,setup,species,models)
    K5_ = mdv.C_CO3(run,PT,melt_wf,setup,species,models)
    KD1_, KD2, y = mdv.FefO2_KC91_EqA_terms(run,PT,melt_wf,setup,species,models)
...
def eq_CHOFe_2(run,PT,bulk_wf,melt_wf,species,setup,models,nr_step,nr_tol,guessx,guessy):
    ...
    K4_ = mdv.C_H2O(run,PT,melt_wf,setup,species,models)
    K5_ = mdv.C_CO3(run,PT,melt_wf,setup,species,models)
    K6_ = mdv.C_H2(run,PT,melt_wf,setup,species,models)/1000000.
    K7_ = mdv.C_CO(run,PT,melt_wf,setup,species,models)/1000000.
    K8_ = mdv.C_CH4(run,PT,melt_wf,setup,species,models)/1000000.
    KD1_, KD2, y = mdv.FefO2_KC91_EqA_terms(run,PT,melt_wf,setup,species,models)
...
def eq_CH(run,PT,bulk_wf,melt_wf,species,setup,models,nr_step,nr_tol,guessx): # H2O
    P = PT["P"]
    wt_C = bulk_wf['C']
    wt_H = bulk_wf['H']
...
def eq_CHOXFe(run,PT,bulk_wf,melt_wf,species,setup,models,nr_step,nr_tol,guessx,guessy,guessz,solve_species):
    ...
    K4_ = mdv.C_H2O(run,PT,melt_wf,setup,species,models)
    K5_ = mdv.C_CO3(run,PT,melt_wf,setup,species,models)
    K6_ = mdv.C_X(run,PT,melt_wf,setup,species,models)/1000000.0
    K11_ = mdv.C_CO(run,PT,melt_wf,setup,species,models)/1000000.0
    K12_ = mdv.C_CH4(run,PT,melt_wf,setup,species,models)/1000000.0
    K13_ = mdv.C_H2(run,PT,melt_wf,setup,species,models)/1000000.0
    KD1_, KD2, y = mdv.FefO2_KC91_EqA_terms(run,PT,melt_wf,setup,species,models)
...
def eq_SCHOFe(run,PT,bulk_wf,melt_wf,species,setup,models,nr_step,nr_tol,guessx,guessy,guessz,solve_species):
    P = PT["P"]
    wt_O = bulk_wf['O']
    wt_S = bulk_wf['S']
    wt_H = bulk_wf['H']
...
# things to possibly incluce...water speciation using an ideal solution model, SO3

# all gas species - oxidised and reduced melt species - H2S #

def eq_SCHOFe_2(run,PT,bulk_wf,melt_wf,species,setup,models,nr_step,nr_tol,guessx,guessy,guessz,solve_species):
    P = PT["P"]
    wt_O = bulk_wf['O']
    wt_S = bulk_wf['S']
    wt_H = bulk_wf['H']
...
def eq_SCHOFe_test(run,PT,bulk_wf,melt_wf,species,setup,models,nr_step,nr_tol,guessx,guessy,guessz,solve_species):
    ...
    K4_ = float(mdv.C_H2O(run,PT,melt_wf,setup,species,models))
    K5_ = float(mdv.C_CO3(run,PT,melt_wf,setup,species,models))
    K6_ = float(mdv.KOSg(PT,models))
    K7_ = float(mdv.KHOSg(PT,models))
    K8_ = float(mdv.C_S(run,PT,melt_wf,setup,species,models)/1000000.0)
    K9_ = float(mdv.C_SO4(run,PT,melt_wf,setup,species,models)/1000000.0)
    K10_ = float(mdv.KOCSg(PT,models))
    K11_ = float(mdv.C_CO(run,PT,melt_wf,setup,species,models)/1000000.0)
    K12_ = float(mdv.C_CH4(run,PT,melt_wf,setup,species,models)/1000000.0)
    K13_ = float(mdv.C_H2(run,PT,melt_wf,setup,species,models)/1000000.0)
    K14_ = float(mdv.C_H2S(run,PT,melt_wf,setup,species,models)/1000000.0)
    KD1_, KD2, y = mdv.FefO2_KC91_EqA_terms(run,PT,melt_wf,setup,species,models)
    KD1_=float(KD1_)
    KD2=float(KD2)
    y=float(y)
    ...
    M_m_ = float(mg.M_m_SO(run,melt_wf,setup,species))
    M_m_ox = float(mg.M_m_ox(run,melt_wf,setup,species,models))
...
def eq_SCHOXFe(run,PT,bulk_wf,melt_wf,species,setup,models,nr_step,nr_tol,guessx,guessy,guessz,guessw,solve_species):
    species_X = models.loc["species X","option"]
    ...
    K13_ = float(mdv.C_H2(run,PT,melt_wf,setup,species,models)/1000000.0)
    K14_ = float(mdv.C_H2S(run,PT,melt_wf,setup,species,models)/1000000.0)
    K15_ = float(mdv.C_X(run,PT,melt_wf,setup,species,models)/1000000.0)
    KD1_, KD2, y = mdv.FefO2_KC91_EqA_terms(run,PT,melt_wf,setup,species,models)
    KD1_=float(KD1_)
    KD2=float(KD2)
    y=float(y)
...
def p_tot_fO2_fS2(run,PT,melt_wf,setup,species,models): 
    ...
    K2 = mdv.C_S(run,PT,melt_wf,setup,species,models)/1000000.
    K3 = mdv.C_SO4(run,PT,melt_wf,setup,species,models)/1000000.
...
def S_P_fO2(run,PT,fO2,melt_wf,setup,species,models):
    ...
    K12_ = mdv.KHOm(run,PT,melt_wf,setup,species,models)
    K13_ = mdv.C_H2(run,PT,melt_wf,setup,species,models)
    KD1_, KD2, y = mdv.FefO2_KC91_EqA_terms(run,PT,melt_wf,setup,species,models)
    ...
    if melt_wf["H2OT"] > 0.:
        ...
        fH2O = mg.f_H2O(run,PT,melt_wf,setup,species,models)
    ...
    return xg_O2_, xg_S2_, xg_SO2_, xg_SO3_, xg_H2_, xg_H2O_, xg_H2S_, xg_CO_, xg_CO2_, xg_CH4_, xg_OCS_, wm_S_, wm_SO3_, wm_ST_, S6T, wm_H2O_, xm_H2O_, wm_H2_, xm_H2_, wm_H2Omol_, xm_H2Omol_, wm_OH_, xm_OH_, wm_CO2_, xm_CO2_, Xg_t, Xm_t, Xm_t_ox, wt_C_, wt_H_, wt_S_, wt_g