...
def test_S2fO2_df_Xsulf():
    ...
    assert result.loc[0, "P (bar) sulf"] == pytest.approx(3223.484043)
    assert result.loc[0, "DFMQ-sulfide"] == pytest.approx(1.022902)
    assert result.loc[0, "P (bar) anh"] == pytest.approx('')
    assert result.loc[0, "DFMQ-sulfate"] == pytest.approx('')
...
def test_S2fO2_df_useropt():
    ...
    assert result.loc[0, "P (bar) sulf"] == pytest.approx(3515.032438)
    assert result.loc[0, "DFMQ-sulfide"] == pytest.approx('')
    assert result.loc[0, "P (bar) anh"] == pytest.approx('')
    assert result.loc[0, "DFMQ-sulfate"] == pytest.approx('')